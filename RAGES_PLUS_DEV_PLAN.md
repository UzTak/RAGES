...
| Prep for item 8: flow-matching model class | Done (class only) | Added `ConditionalFlowMatcher` to `src/wyp_predictor.py`, following the GMM/VAE protocol (`forward` returns conditioning; `compute_loss` does the CFM regression with reward weighting and `m_missing` masking; `sample_y` integrates the learned ODE with `cfg.flow_steps` Euler steps). dt flows in logit space (log fractions) and is finalized by `constrained_fill`, matching the GMM/VAE output convention; `use_mean_w=True` decodes deterministically from the zero base sample (cf. Sec. 4.4). `load_model` supports `model_type="flow"`; `FillerConfig` gained `flow_steps` (default 32, backward compatible). Validated on a toy conditional task: loss convergence, deterministic-decode repeatability, simplex recovery, sample diversity, flow checkpoint round-trip, and legacy GMM checkpoint loading. Training script `work/train_wyp_flow.py` (item 8) not started. |
